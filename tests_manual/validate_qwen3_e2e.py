"""Qwen3 Forced Aligner Phase-3-bundle end-to-end validation (GPU).

Validates the cjm-hf-plugin-utils adoption (HFCacheConfig mixin +
snapshot_download_with_progress + load_pretrained_with_oom) and cjm-torch-plugin-utils
(release_model + cuda_oom), the heartbeat-wrapped HF Hub model load, the Track 19
WORKER_ENV migration (XDG_CACHE_HOME -> templated HF_HOME), and the self._config ->
self.config rename live, mirroring the Voxtral-HF Phase 3 pattern.

Run from the qwen3 repo root after:
  1. `cjm-ctl --cjm-config cjm.yaml setup-runtime`
  2. `cjm-ctl --cjm-config cjm.yaml install-all --plugins plugins_test.yaml --force`
     (qwen3 + ffmpeg + cjm-system-monitor-nvidia)
  3. A short speech clip + its transcript at
     test_files/short_test_audio.mp3 + short_test_audio.txt

Then:
  conda run -n cjm-transcription-plugin-qwen3-forced-aligner --no-capture-output \\
    python tests_manual/validate_qwen3_e2e.py
"""
import json
import logging
import sqlite3
...
log = logging.getLogger("qwen3-e2e")

REPO_ROOT = Path(__file__).resolve().parent.parent
TEST_AUDIO = REPO_ROOT / "test_files" / "short_test_audio.mp3"
TEST_TEXT = REPO_ROOT / "test_files" / "short_test_audio.txt"
...
def check_prereqs() -> None:
    ...
    assert TEST_TEXT.exists(), f"Missing test transcript: {TEST_TEXT}"
    ...
    log.info("Prereqs OK: audio + transcript + qwen3 + nvidia-monitor + ffmpeg manifests present")
...
def assert_manifest_shape() -> None:
    ...
    tax = code["taxonomy"]
    assert tax["domain"] == "transcription" and tax["role"] == "ForcedAlignmentPlugin", tax
    ...
    # Track 19: static CUDA_VISIBLE_DEVICES + OMP_NUM_THREADS + templated HF_HOME (was XDG_CACHE_HOME).
    ...
    assert "XDG_CACHE_HOME" not in by_name, "XDG_CACHE_HOME should have been replaced by HF_HOME"
    hf_home_default = by_name["HF_HOME"].get("default", "")
    assert hf_home_default == "${CJM_MODELS_DIR}/huggingface", f"HF_HOME default not templated: {hf_home_default!r}"
    ...
    # HFCacheConfig mixin should have added cache_dir/revision/local_files_only/trust_remote_code to the schema.
    props = code.get("config_schema", {}).get("properties", {})
    for f in ("cache_dir", "revision", "local_files_only", "trust_remote_code"):
        assert f in props, f"HFCacheConfig field {f!r} missing from config_schema"
    log.info("HFCacheConfig mixin fields present in config_schema")
...
def run_e2e() -> None:
    ...
    transcript = TEST_TEXT.read_text().strip()
    cfg = get_config()
    ...
    qwen3_meta = next(m for m in pm.discovered if m.name == PLUGIN_NAME)
    db_path = qwen3_meta.manifest.get("db_path")
    ok = pm.load_plugin(qwen3_meta, config={})
    assert ok, f"Failed to load {PLUGIN_NAME}"
    qwen3_id = qwen3_meta.name
    log.info(f"Loaded {SYSMON_NAME} + {FFMPEG_NAME} + {PLUGIN_NAME}; db_path={db_path}")

    # CR-4 prefetch: HF Hub snapshot download + heartbeat-wrapped from_pretrained.
    log.info("Calling prefetch() to download + load the Qwen3 aligner model...")
    t0 = time.time()
    pm.get_plugin(qwen3_id).prefetch()
    ...
    async def run_composition() -> Any:
        ...
        try:
            comp_id = await queue.submit_composition(Composition(nodes=[
                ...
                CompositionNode("align", qwen3_id, {
                    "audio": OutputRef("convert", "output_path"),
                    "text": transcript,
                }),
            ]))
            log.info(f"Submitted composition {comp_id}: ffmpeg.convert -> qwen3.align")
            ...
            return run.results_by_node()["align"]
        finally:
            await queue.stop()

    log.info(f"Submitting composition for {TEST_AUDIO.name} ({len(transcript)} chars)...")
    t0 = time.time()
    ...
    try:
        for t in [r[0] for r in con.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()]:
            ...
            for r in con.execute(
                ...
                (PLUGIN_NAME, qwen3_id, f"{PLUGIN_NAME}%"),
            ).fetchall():
                row = dict(zip(cols, r))
    ...
    log.info(f"GPU attribution VERIFIED: qwen3 gpu_memory_mb_peak_max={gpu_peak:.1f} MB")

    pm.unload_plugin(qwen3_id)
